...
config = {
	enable = 1,
	spawn = {"BOTTOMLEFT", UIParent, "BOTTOMLEFT", 47, 13},
	width = 378,
	height = 7,
	color = {.4, .1, 0.6, 1},
...
bar:SetScript("OnLeave", function() xpbar.text:SetAlpha(0) end)
--MakeMovable(bar)
...
local numberize = function(v)
	...
	if v >= 1000000 then
		...
		return value
	elseif v >= 1000 then
		local value = string.format("%.1fk", v/100)
		return value
	end
end
...
xpbar.text:SetPoint("CENTER", bar, "CENTER", 0, 0)
xpbar.text:SetFont(media.font, 12, "OUTLINE")
