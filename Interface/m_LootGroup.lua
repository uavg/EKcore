local bartex = "Interface\\TargetingFrame\\UI-StatusBar"  --條材質
local fontn = STANDARD_TEXT_FONT  --字體
local edgetex = "Interface\\Tooltips\\UI-Tooltip-Border"
local loottex =	"Interface\\QuestFrame\\UI-QuestLogTitleHighlight"
local blanktex = 	"Interface\\Buttons\\WHITE8x8"
local fontsize = 16  --字號
local iconsize = 28 					-- loot frame icon's size
...
local function SetTip(frame)
	...
	if roll == rolltypes[frame.rolltype] then GameTooltip:AddLine(name, 1, 1, 1) end end
	GameTooltip:Show()
end
...
local function CreateRollButton(parent, ntex, ptex, htex, rolltype, tiptext, ...)
	...
	txt:SetFont(fontn, fontsize, "OUTLINE")
...
local function CreateRollFrame()
	...
	button:SetPoint("LEFT", -iconsize, iconsize/4)
	...
	buttonborder:SetWidth(iconsize)
	buttonborder:SetHeight(iconsize)
	...
	buttonborder2:SetWidth(iconsize+2)
	buttonborder2:SetHeight(iconsize+2)
	...
	status:SetWidth(bar_width)
	status:SetHeight(bar_height-2)
	status:SetPoint("LEFT", frame, "LEFT", 0, 0)
	status:SetFrameLevel(status:GetFrameLevel()-1)
	status:SetStatusBarTexture(bartex)
	...
	spark:SetWidth(14)
	spark:SetHeight(bar_height+3)
	spark:SetTexture("Interface\\CastingBar\\UI-CastingBar-Spark")
	spark:SetBlendMode("ADD")
	status.spark = spark

	local need, needtext = 	CreateRollButton(frame, 
	"Interface\\Buttons\\UI-GroupLoot-Dice-Up", "Interface\\Buttons\\UI-GroupLoot-Dice-Highlight", 
	"Interface\\Buttons\\UI-GroupLoot-Dice-Down", 1, NEED, 	"BOTTOMLEFT", frame.status, "BOTTOMLEFT", 5, -3)
	local greed, greedtext = CreateRollButton(frame, 
	"Interface\\Buttons\\UI-GroupLoot-Coin-Up", "Interface\\Buttons\\UI-GroupLoot-Coin-Highlight", 
	"Interface\\Buttons\\UI-GroupLoot-Coin-Down", 2, GREED, "LEFT", need, "RIGHT", 0, -1)
	local pass, passtext = CreateRollButton(frame, 
	"Interface\\Buttons\\UI-GroupLoot-Pass-Up", nil,
	"Interface\\Buttons\\UI-GroupLoot-Pass-Down", 0, PASS, "BOTTOMRIGHT", frame.status, "BOTTOMRIGHT", -3, 1.4)
	local de, detext
	de, detext = CreateRollButton(frame, 
	"Interface\\Buttons\\UI-GroupLoot-DE-Up", "Interface\\Buttons\\UI-GroupLoot-DE-Highlight", 
	"Interface\\Buttons\\UI-GroupLoot-DE-Down", 3, ROLL_DISENCHANT, "RIGHT", pass, "LEFT", 0, -3)
	frame.needbutt, frame.greedbutt, frame.disenchantbutt = need, greed, de
	frame.need, frame.greed, frame.pass, frame.disenchant = needtext, greedtext, passtext, detext

	local bind = frame:CreateFontString()
	bind:SetPoint("LEFT", greed, "RIGHT", 3, 1)
	bind:SetFont(fontn, fontsize, "OUTLINE")
	frame.fsbind = bind

	local loot = frame:CreateFontString(nil, "ARTWORK")
	loot:SetFont(fontn, fontsize, "OUTLINE")
	loot:SetPoint("LEFT", bind, "RIGHT", 0, 0)
	loot:SetPoint("RIGHT", de, "LEFT", -5, 0)
	loot:SetHeight(10)
	loot:SetWidth(bar_width-bar_height*4)
...
local LootRollAnchor = CreateFrame("Button", "LootRollAnchor", UIParent)
	LootRollAnchor:SetWidth(bar_width) 
	LootRollAnchor:SetHeight(bar_height)
	LootRollAnchor:SetBackdrop(backdrop)
	LootRollAnchor:SetBackdropColor(0.25, 0.25, 0.25, 1)
	LootRollAnchor:SetPoint(unpack(position))
local label = LootRollAnchor:CreateFontString(nil, "ARTWORK")
	label:SetFont(fontn, fontsize, "OUTLINE")
...
f:SetPoint("BOTTOMLEFT", next(frames) and frames[#frames] or LootRollAnchor, "TOPLEFT", 0, barspacing)
...
SlashCmdList.TESTROLL = function()
	...
	if f:IsShown() then
		...
		f:SetBackdropBorderColor(r, g, b, 0)
		f.need:SetText(0)
		f.greed:SetText(0)
		f.pass:SetText(0)
		f.disenchant:SetText(0)
